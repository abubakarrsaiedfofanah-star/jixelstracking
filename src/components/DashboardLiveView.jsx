...
import { AlertTriangle, Bell, ClipboardCheck, ClipboardList, MessageSquarePlus, PackagePlus, Radio, UserRoundPlus } from "lucide-react";
...
export default function DashboardLiveView({ onStart = () => {} }) {
  ...
  const legacyCards = [
    { label: "Products", value: products, detail: "All registered product types", tone: "ink" },
    { label: "Open cases", value: cases.filter((item) => !["closed", "resolved"].includes(item.status)).length, detail: "Support cases needing attention", tone: "warning" }
  ];
  const quickActions = [
    { label: "Add customer", detail: "Create a customer profile", page: "Customers", icon: UserRoundPlus },
    { label: "Register product", detail: "Link a product and tracker", page: "Products", icon: PackagePlus },
    { label: "Start screening", detail: "Record a new application", page: "Screening", icon: ClipboardCheck },
    { label: "Open support case", detail: "Log an issue for follow-up", page: "Support Cases", icon: MessageSquarePlus }
  ];

  return <>
    <section className="dashboard-quick-actions" aria-label="Common tasks"><div className="quick-actions-heading"><div><h2>Start a task</h2><p>Choose the action that matches what you need to do.</p></div></div><div className="quick-actions-grid">{quickActions.map((action) => <button className="quick-action" key={action.label} onClick={() => onStart(action.page)}><span className="quick-action-icon"><action.icon size={18} /></span><span><strong>{action.label}</strong><small>{action.detail}</small></span></button>)}</div></section>
    ...
    <section className="dashboard-legacy-summary" aria-label="Operational summary">{legacyCards.map((card) => <article className={`legacy-overview-card ${card.tone}`} key={card.label}><div><span>{card.label}</span><strong>{card.value}</strong></div><small>{card.detail}</small></article>)}</section>
